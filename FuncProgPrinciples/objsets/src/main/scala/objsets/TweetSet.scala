...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def descendingByRetweet: TweetList = {
      def loop(xl: TweetList, xs: TweetSet): TweetList = {
        if(xs.isEmpty) xl
        else {
          val tweet: Tweet = xs.mostRetweeted
          loop(new Cons(tweet, xl), xs.remove(tweet))   
        }
      }
      
      def reverseList(xl: TweetList, xres: TweetList): TweetList = {
        if(xl.isEmpty) xres
        else
          reverseList(xl.tail, new Cons(xl.head, xres))
      }
      
      reverseList(loop(Nil, this), Nil)
    }     
    ...
    def mostRetweeted: Tweet = 
      if(left.isEmpty && right.isEmpty) elem
        ...
        val leftMost: Tweet = left.mostRetweeted
        val rightMost: Tweet = right.mostRetweeted
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = 
    tweets.filter(t => google.exists(e => t.text.contains(e)))

  lazy val appleTweets: TweetSet = 
    tweets.filter(t => apple.exists(e => t.text.contains(e)))
